...
const btnViewSOPDF = document.getElementById("btnViewSOPDF");
const btnPostSalesOrder = document.getElementById("btnPostSalesOrder");
const btnCreateDeliveryReceipt = document.getElementById(
	"btnCreateDeliveryReceipt",
);
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	Atlas.select.onChange("#selCustomer", (option) => {
		...
		customerDiscountValue = Atlas.format.parseNumber(
			option.dataset.discountValue || 0,
		);
		markDirty();
	});
	...
	/*** new */
	btnNewSalesOrder?.addEventListener("click", () =>
		Atlas.page.redirect(`sales-orders/create/`),
	);
	...
	btnSaveSalesOrder?.addEventListener("click", async () => {
		...
		try {
			...
			document.querySelectorAll("#tblSalesOrderDetails tr").forEach((row) => {
				...
				const uomId = Atlas.format.parseNumber(
					row.querySelector(".so-uom").value,
				);
				if (!uomId) {
				...
				if (!conversionFactor || conversionFactor <= 0) {
					Atlas.toast.error(
						"Please provide a valid UOM conversion for all products.",
					);
					return;
				}
				...
				salesOrder.details.push({
					...
					unit_price: Atlas.format.parseNumber(
						row.querySelector(".so-unit-price").value,
					),
					discount_type: row.querySelector(".so-discount-type").value || null,
					discount_value: Atlas.format.parseNumber(
						row.querySelector(".so-discount-value").value || 0,
					),
					discount_amount: Atlas.format.parseNumber(
						row.dataset.discountAmount || 0,
					),
				});
			});

			const result = await Atlas.ajax.post("sales-orders/save", salesOrder);
			...
			setTimeout(
				() =>
					Atlas.page.redirect(
						`sales-orders/edit/${Atlas.id.encode(result.data.sales_order_id)}`,
					),
				1500,
			);
		} finally {
	...
	btnPostSalesOrder?.addEventListener("click", async () => {
		...
		try {
			const response = await Atlas.ajax.post("sales-orders/post", {
				ids: ids,
			});
	...
	btnCreateDeliveryReceipt?.addEventListener("click", () => {
		...
		if (!row) {
			...
			if (status !== "POSTED") {
				Atlas.toast.warning(
					"Only POSTED Sales Orders can create a Delivery Receipt.",
				);
				return;
			}
			...
			Atlas.page.redirectRemember(
				`delivery-receipts/create/${Atlas.id.encode(window.salesOrderId)}`,
			);
		}
	...
	/*** view as PDF */
	btnViewSOPDF?.addEventListener("click", viewSalesOrderPDF);
	...
	btnCancelSalesOrder?.addEventListener("click", async () => {
		...
		const result = await Atlas.ajax.post("sales-orders/cancel", {
			ids: ids,
	...
	btnDownloadSOExcel?.addEventListener("click", () => {
		Atlas.excel.download(document.getElementById("tblSOList"), {
	...
	btnRefreshSalesOrder?.addEventListener("click", () =>
		Atlas.page.redirect(`sales-orders`),
	);
	...
	document.addEventListener("change", async (e) => {
		...
		const baseUomOption = [...e.target.options].find(
	...
	tblSalesOrderDetails
		?.querySelectorAll("tr[data-product-id]")
		.forEach((row) => {
	...
	if (tblSalesOrderDetails) calculateSalesOrderTotals();
...
const populateProductRow = (row, product) => {
	...
	const duplicate = [...tblSalesOrderDetails.rows].find(
		(r) =>
			r !== row &&
			Atlas.format.integer(r.dataset.productId) ===
				Atlas.format.integer(product.id),
	);
	if (duplicate) {
	...
	discountValue.value = Atlas.format
		.parseNumber(customerDiscountValue || 0)
		.toFixed(2);
...
const calculateSalesOrderRow = (row) => {
	...
	const unitPrice = Atlas.format.parseNumber(
		row.querySelector(".so-unit-price")?.value || 0,
	);
	const discountType = row.querySelector(".so-discount-type")?.value || "";

	let discountValue = Atlas.format.parseNumber(
		row.querySelector(".so-discount-value")?.value || 0,
	);
	...
	row.querySelector(".so-net-amount").textContent =
		Atlas.format.amount(netAmount);
...
const calculateSalesOrderTotals = () => {
	...
	document.getElementById("soGrossAmount").textContent =
		Atlas.format.amount(grossAmount);
	document.getElementById("soDiscountAmount").textContent =
		Atlas.format.amount(discountAmount);
	document.getElementById("soSubtotal").textContent =
		Atlas.format.amount(subtotal);
	document.getElementById("soVatRateLabel").textContent =
		`${vatRate.toFixed(2)}%`;
	document.getElementById("soVatAmount").textContent =
		Atlas.format.amount(vatAmount);
	document.getElementById("soTotalAmount").textContent =
		Atlas.format.amount(totalAmount);
};

const renumberRows = () => {
	document
		.querySelectorAll("#tblSalesOrderDetails tr")
		.forEach((row, index) => {
...
const validateSalesOrder = () => {
	...
	for (let i = 0; i < rows.length; i++) {
		...
		/*** validate conversion */
		const conversionFactor = Atlas.format.parseNumber(
			row.dataset.conversionFactor || 0,
		);
		...
		if (qty > qtyAvailable) {
			Atlas.toast.warning(
				`Insufficient stock on row ${i + 1}. ` +
					`Available: ${Atlas.format.amount(qtyAvailable)}`,
			);
			setTimeout(() => row.querySelector(".so-qty").focus(), 500);
			return false;
		}
...
const getSelectedSalesOrderId = () => {
	...
	if (checkedRow.length > 1) {
		Atlas.toast.warning("Please select only one Sales Order.");
...
const viewSalesOrderPDF = () => {
	...
	Atlas.print.post("sales-orders/pdf", ids);
};
