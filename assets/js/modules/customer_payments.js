...
const btnEditCustomerPayment = document.getElementById(
	"btnEditCustomerPayment",
);
const btnPostCustomerPayment = document.getElementById(
	"btnPostCustomerPayment",
);
...
const btnSaveCustomerPayment = document.getElementById(
	"btnSaveCustomerPayment",
);

const tblOutstandingInvoices = document.getElementById(
	"tblOutstandingInvoices",
);
const tblAvailableCredits = document.getElementById("tblAvailableCredits");
const tblCustomerCreditRefunds = document.getElementById(
	"tblCustomerCreditRefunds",
);
const tblCustomerPaymentDeductions = document.getElementById(
	"tblCustomerPaymentDeductions",
);
const btnAddCustomerPaymentDeduction = document.getElementById(
	"btnAddCustomerPaymentDeduction",
);
...
const txtCustomerPaymentRemarks = document.getElementById(
	"txtCustomerPaymentRemarks",
);
const dtPaymentDate = document.getElementById("dtPaymentDate");

const cpOtherDeductions = document.getElementById("cpOtherDeductions");
const cpTotalSettlement = document.getElementById("cpTotalSettlement");
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	if (window.branchId) $("#selBranch").val(branchId).trigger("change");

	/*** dirty tracking, clear validation */
	dtPaymentDate?.addEventListener("change", () => clearInvalid(dtPaymentDate));
	txtAmountReceived?.addEventListener("input", () =>
	...
	btnSaveCustomerPayment?.addEventListener("click", async () => {
		...
		const customerPayment = {
			...
			collected_by_salesman_id: Atlas.format.parseNumber(
				selCollectedBy.value || 0,
			),
		...
		document
			.querySelectorAll(
				"#tblOutstandingInvoices tbody tr[data-sales-invoice-id]",
			)
			.forEach((row) => {
				...
				customerPayment.allocations.push({
					sales_invoice_id: Atlas.format.parseNumber(
						row.dataset.salesInvoiceId,
					),
		...
		document
			.querySelectorAll("#tblCustomerPaymentDeductions tbody tr")
			.forEach((row) => {
				const invoice = row.querySelector(".selDeductionInvoice");
				const particulars = row.querySelector(".txtDeductionParticulars");
				const amount = row.querySelector(".txtDeductionAmount");

				if (!invoice || !particulars || !amount) return;

				customerPayment.deductions.push({
					sales_invoice_id: Atlas.format.parseNumber(invoice.value || 0),
					particulars: particulars.value.trim(),
					amount: Atlas.format.parseNumber(amount.value || 0),
				});
			});
		...
		try {
			...
			setTimeout(
				() =>
					Atlas.page.redirect(
						`customer-payments/edit/${Atlas.id.encode(result.data.customer_payment_id)}`,
					),
				1500,
			);
		} finally {
	...
	btnPostCustomerPayment?.addEventListener("click", async () => {
		...
		try {
			const response = await Atlas.ajax.post("customer-payments/post", {
				ids: ids,
			});
	...
	btnCancelCustomerPayment?.addEventListener("click", async () => {
		...
		try {
			const response = await Atlas.ajax.post("customer-payments/cancel", {
				ids: ids,
	...
	/*** other deductions */
	btnAddCustomerPaymentDeduction?.addEventListener("click", () => {
		addCustomerPaymentDeductionRow();
	});

	/*** remove deduction */
	tblCustomerPaymentDeductions?.addEventListener("click", (e) => {
		const button = e.target.closest(".btnRemoveCustomerPaymentDeduction");
		if (!button) return;

		button.closest("tr")?.remove();
		updateCustomerPaymentDeductionEmptyRow();
		calculatePaymentAllocationTotals();
		markDirty();
	});

	tblCustomerPaymentDeductions?.addEventListener("input", (e) => {
		if (
			e.target.classList.contains("txtDeductionParticulars") ||
			e.target.classList.contains("txtDeductionAmount")
		) {
			markDirty();

			if (e.target.classList.contains("txtDeductionAmount")) {
				calculatePaymentAllocationTotals();
			}
		}
	});

	tblCustomerPaymentDeductions?.addEventListener("change", (e) => {
		if (e.target.classList.contains("selDeductionInvoice")) {
			markDirty();
		}
	});
	...
	Atlas.select.onChange("#selCustomer", async () => {
		...
		if (tblCustomerPaymentDeductions) {
			tblCustomerPaymentDeductions.querySelector("tbody").innerHTML = `
	...
	/*** refresh */
	btnRefreshCustomerPayment?.addEventListener("click", () =>
	...
	if (window.customerPaymentId > 0 && selCustomer?.value) {
		...
		(window.customerPaymentDeductions || []).forEach((deduction) => {
			addCustomerPaymentDeductionRow(deduction);
		});
	...
	tblAvailableCredits?.addEventListener("click", async (e) => {
		...
		const creditBalance = Atlas.format.parseNumber(
			row.dataset.creditBalance || 0,
		);
		...
		if (!salesInvoiceId) {
			Atlas.toast.warning(
				"Please select the Sales Invoice to apply the credit to.",
			);
			select?.focus();
			return;
		}
		...
		const invoiceBalance = Atlas.format.parseNumber(
		...
		if (amount > creditBalance) {
			Atlas.toast.warning(
				"Amount cannot exceed the available customer credit.",
			);
			input?.focus();
			return;
		}
	...
	tblCustomerCreditRefunds?.addEventListener("click", async (e) => {
		...
		const reason = await Atlas.dialog.textarea({
            ...
            `,
			inputPlaceholder:
				"Enter cancellation reason, for audit purposes, this field is required....",
			required: true,
	...
	tbody.innerHTML = invoices
		.map((invoice) => {
			const savedAllocation = savedAllocations.find(
				(allocation) =>
					Number(allocation.sales_invoice_id) === Number(invoice.id),
			);

			const amountApplied = savedAllocation
				? Number(savedAllocation.amount_applied)
				: 0;
			const amountValue =
				amountApplied === 0 ? "" : `value="${amountApplied.toFixed(2)}"`;
			const creditAmount =
				Atlas.format.parseNumber(invoice.amount_credited || 0) +
				Atlas.format.parseNumber(invoice.credit_applied || 0);
...
const loadAvailableCredits = async () => {
	...
	const paymentCredits = (response.data?.payment_credits || []).map(
		(credit) => ({
			type: "CP",
			id: credit.id,
...
const addCustomerPaymentDeductionRow = (deduction = null) => {
	if (!tblCustomerPaymentDeductions) return;
	...
	if (!selCustomer?.value) {
		Atlas.toast.warning("Please select a Customer first.");
		return;
	}

	if (!invoiceRows.length) {
		Atlas.toast.warning(
			"No outstanding Sales Invoice available for deduction.",
		);
		return;
	}

	const tbody = tblCustomerPaymentDeductions.querySelector("tbody");
	tbody.querySelector(".deduction-empty-row")?.remove();

	const options = Array.from(invoiceRows)
		.map((row) => {
			const invoiceId = row.dataset.salesInvoiceId;
			const invoiceNo = row.children[0]?.textContent.trim() || "";
			const balance = Atlas.format.parseNumber(row.dataset.balance || 0);
			const selected =
				Number(deduction?.sales_invoice_id) === Number(invoiceId)
					? "selected"
					: "";

			return `<option value="${invoiceId}" ${selected}>${escapeHtml(invoiceNo)} - ${Atlas.format.amount(balance)}</option>`;
		})
		.join("");
    ...
    <tr>
      <td>
        <select class="form-control form-control-sm custom-select selDeductionInvoice">
          <option value="">Select SI</option>
          ${options}
        </select>
      </td>
      <td>
        <input type="text" class="form-control form-control-sm text-uppercase txtDeductionParticulars font-sm" placeholder="Enter particulars / type" value="${escapeHtml(deduction?.particulars || "")}">
      </td>
      <td>
        <input type="number" step="0.01" min="0" class="form-control form-control-sm text-right txtDeductionAmount font-sm" placeholder="0.00" value="${deduction ? Number(deduction.amount).toFixed(2) : ""}">
      </td>
      <td class="text-center">
        <i class="fas fa-trash text-danger pointer btnRemoveCustomerPaymentDeduction" title="Remove Deduction" data-toggle="tooltip"></i>
      </td>
    </tr>
  `,
	);
...
const updateCustomerPaymentDeductionEmptyRow = () => {
	if (!tblCustomerPaymentDeductions) return;

	const tbody = tblCustomerPaymentDeductions.querySelector("tbody");
	if (tbody.querySelector(".selDeductionInvoice")) return;
...
const validateApplyAmount = (input) => {
	...
	const balance =
		Math.round(Atlas.format.parseNumber(row.dataset.balance || 0) * 100) / 100;
	let amount =
		Math.round(Atlas.format.parseNumber(input.value || 0) * 100) / 100;
	if (amount < 0) {
		amount = 0;
		input.value = "0.00";
		return;
...
const calculatePaymentAllocationTotals = () => {
	const amountReceived = Atlas.format.parseNumber(
	...
	document.querySelectorAll(".txtDeductionAmount").forEach((input) => {
		otherDeductions += Atlas.format.parseNumber(input.value || 0);
	});
	const totalSettlement = amountApplied + otherDeductions;

	cpAmountReceived.textContent = Atlas.format.amount(amountReceived);
	cpAmountApplied.textContent = Atlas.format.amount(amountApplied);
	cpOtherDeductions.textContent = Atlas.format.amount(otherDeductions);
	cpTotalSettlement.textContent = Atlas.format.amount(totalSettlement);
	cpAmountUnapplied.textContent = Atlas.format.amount(unapplied);
};
...
const validateCustomerPayment = () => {
	...
	document
		.querySelectorAll("#tblOutstandingInvoices tbody tr[data-sales-invoice-id]")
		.forEach((row) => {
			if (!valid) {
				return;
			}
			...
			if (amountApplied > balance) {
				Atlas.toast.warning(
					"Applied amount cannot exceed the invoice balance.",
				);
				input.focus();
				valid = false;
				return;
