...
const btnSaveChangesGoodsReceipt = document.getElementById(
	"btnSaveChangesGoodsReceipt",
);
const tblGoodsReceiptDetails = document.getElementById(
	"tblGoodsReceiptDetails",
);
const btnPrintGoodsReceipt = document.getElementById("btnPrintGoodsReceipt");
const btnRefreshGoodsReceipt = document.getElementById(
	"btnRefreshGoodsReceipt",
);
...
const btnCreatePurchaseReturn = document.getElementById(
	"btnCreatePurchaseReturn",
);
const btnFillReceiveNow = document.getElementById("btnFillReceiveNow");
...
document.addEventListener("DOMContentLoaded", () => {
	...
	/*** fill receive now */
	btnFillReceiveNow?.addEventListener("click", fillReceiveNow);

	/*** update */
	btnSaveChangesGoodsReceipt?.addEventListener(
		"click",
		saveChangesGoodsReceipt,
	);
	...
	btnCreatePurchaseReturn?.addEventListener("click", () => {
		...
		const status =
			window.status || Atlas.table.selected()[0]?.closest("tr").dataset.status;
		...
		if (!ids || ids.length !== 1) {
			...
			Atlas.page.redirectRemember(
				`purchase-returns/create/${Atlas.id.encode(window.goodsReceiptId)}`,
			);
			return;
		}
		Atlas.page.redirectRemember(
			`purchase-returns/create/${Atlas.id.encode(ids[0])}`,
		);
	});

	/*** refresh */
	btnRefreshGoodsReceipt?.addEventListener("click", () =>
		Atlas.page.redirectRemember(`goods-receipts`),
	);
...
const collectReceiptDetails = () => {
	...
	tblGoodsReceiptDetails.querySelectorAll("tbody tr").forEach((row) => {
		const receiveNow = Atlas.format.parseNumber(
		...
		const conversionFactor = Atlas.format.parseNumber(
			row.querySelector(".grn-conversion").value || 0,
		);
		...
		const originalConversion = Atlas.format.parseNumber(
			row.dataset.originalConversion || 0,
		);
...
const saveGoodsReceipt = async () => {
	...
	try {
		...
		formData.append(
			"po_id",
			document.getElementById("hidPurchaseOrderId").value,
		);
		formData.append(
			"supplier_id",
		...
		const result = await Atlas.ajax.post("goods-receipts/save", formData);
		...
		setTimeout(
			() =>
				Atlas.page.redirect(
					`goods-receipts/view/${Atlas.id.encode(result.data.goods_receipt_id)}`,
				),
			1500,
		);
	} finally {
...
const saveChangesGoodsReceipt = async () => {
	...
	try {
		...
		document
			.querySelectorAll("#tblGoodsReceiptDetails tbody tr")
			.forEach((row) => {
				...
				grn.details.push({
					...
					qty_received: Atlas.format.parseNumber(
						row.querySelector(".grn-qty").value,
					),
					conversion_factor: conversionFactor,
				});
			});
		...
		const result = await Atlas.ajax.post("goods-receipts/update", grn);
...
const postGoodsReceipt = async () => {
	...
	try {
		const response = await Atlas.ajax.post("goods-receipts/post", {
...
const cancelGoodsReceipt = async () => {
	...
	try {
		const response = await Atlas.ajax.post("goods-receipts/cancel", {
			id: Atlas.format.parseNumber(hidGoodsReceiptId.value),
...
const getSelectedGoodsReceiptId = () => {
	...
	if (checked.length > 1) {
		Atlas.toast.warning("Please select only one Goods Receipt.");
...
const fillReceiveNow = () => {
	document.querySelectorAll(".grn-receive-now").forEach((input) => {
		const row = input.closest("tr");
		input.value = Atlas.format.parseNumber(row.dataset.remainingQty);
	});
