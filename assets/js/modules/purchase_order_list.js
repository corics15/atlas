document.addEventListener("DOMContentLoaded", () => {
	...
	const btnPrintPurchaseOrder = document.getElementById(
		"btnPrintPurchaseOrder",
	);
	const btnCancelPurchaseOrder = document.getElementById(
		"btnCancelPurchaseOrder",
	);
	const btnRefreshPurchaseOrder = document.getElementById(
		"btnRefreshPurchaseOrder",
	);
	const btnClosePurchaseOrder = document.getElementById(
		"btnClosePurchaseOrder",
	);
	...
	btnReceiveGoods.addEventListener("click", () => {
		...
		if (!["OPEN", "PARTIAL"].includes(status)) {
			Atlas.toast.warning(
				"Only OPEN or PARTIAL Purchase Orders can receive goods.",
			);
			return;
		}
	...
	btnRefreshPurchaseOrder.addEventListener("click", () =>
		Atlas.page.redirect(`purchase-orders/list`),
	);
	...
	btnCancelPurchaseOrder.addEventListener("click", async () => {
		...
		const allOpen = selected.every((chk) => chk.dataset.status === "OPEN");

		if (!allOpen) {
			Atlas.toast.warning("Please select only OPEN Purchase Orders to cancel.");
			return;
		}
	...
	btnClosePurchaseOrder.addEventListener("click", async () => {
		...
		const closeReason = await Atlas.dialog.textarea({
...
const getSelectedPurchaseOrderStatus = () => {
	...
	return checked ? checked.dataset.status : null;
};
...
const updateToolbarState = (selected = Atlas.table.selected()) => {
	...
	btnClosePurchaseOrder.disabled = true;
	...
	if (selected.length === 1) {
		...
		if (status === "PARTIAL") {
			btnReceiveGoods.disabled = false;
			btnClosePurchaseOrder.disabled = false;
		}
	}
