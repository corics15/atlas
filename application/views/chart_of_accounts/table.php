  ...
  <?php else: ?>
    ...
    <?php foreach ($accounts as $row): ?>
      <tr data-id="<?= $row->id; ?>">
        ...
        <td class="<?= $row->is_posting == 't' ? 'pl-4' : 'font-weight-500 letter-spacing-2 text-orange' ?>"><?= htmlspecialchars($row->account_name); ?></td>
        <td class="text-center"><?= htmlspecialchars($row->account_type); ?></td>
        <td class="text-center"><?= htmlspecialchars($row->normal_balance); ?></td>
