<?php foreach ($documents as $i => $document): ?>
  ...
  <div class="report">
    ...
    <table class="table report-borderless table-sm mb-3">
      ...
      <tr>
        <td width="150"><strong>SR No.</strong></td>
        <td><?= htmlspecialchars($header->sr_no) ?></td>
        <td width="130"><strong>Return Date</strong></td>
        <td width="170"><?= date('m/d/Y', strtotime($header->return_date)) ?></td>
      </tr>
      <tr>
      ...
      <tbody>
        <?php foreach ($details as $index => $detail): ?>
          ...
          <tr>
            ...
            <td class="text-center" style="white-space:nowrap;"><?= htmlspecialchars($detail->barcode) ?></td>
            <td><?= htmlspecialchars($detail->description) ?></td>
            <td class="text-right" style="white-space:nowrap;"><?= number_format((float)$detail->qty, 2 ) ?></td>
            <td class="text-center" style="white-space:nowrap;"><?= htmlspecialchars($detail->uom) ?></td>
            <td class="text-right" style="white-space:nowrap;"><?= number_format((float)$detail->unit_price, 2) ?></td>
            ...
            <td class="text-right" style="white-space:nowrap;"><?= number_format($rowDiscount, 2) ?></td>
            <td class="text-right" style="white-space:nowrap;"><?= number_format($rowNet, 2) ?></td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
